...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/12/02 17:04:12 by jmartel           #+#    #+#             */
/*   Updated: 2018/12/02 19:48:29 by jmartel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			fillit6(void)
{
	...
	if (pid_test == 0)
	{
		...
		params[1] = strdup(FILLIT6_SAMPLE);
		params[2] = NULL;
	...
	else
	{
		...
		read(pipefd[0], buf, FILLIT6_LEN);
		close(pipefd[0]);
		buf[FILLIT6_LEN] = 0;
		if (strcmp(buf, FILLIT6_RES) == 0)
			exit(SUCCESS);
		else
			exit(FAILURE);
	}
