...
/*   Updated: 2019/01/11 20:13:49 by jmartel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void static	ft_strrep_free_free(char *s1, char *s2, char *pattern, int  param)
{
	if (param & 0x0001)
		free(s1);
	if (param & 0x0002)
...
char		*ft_strrep_free(char *s1, char *s2, char *pattern, int param)
{
	...
	if (!(pt = ft_strstr(s1, pattern)))
		return (NULL);
	if (!(res = malloc(sizeof(*res) * (ft_strlen(s1) + ft_strlen(s2) - ft_strlen(pattern) + 1))))
		return (NULL);
	i1 = -1;
	while (s1 + ++i1 != pt)
		res[i1] = s1[i1];
